...
class CausalSelfAttention(nn.Module):
    def __init__(self, d_model, n_heads, max_seq_len, dropout=0.1):
        ...
        # Precomputed causal mask (lower triangular)
        # Ensures each position only attends to itself and previous positions
    ...
    def forward(self, x):
        ...
        # Mask is broadcast across batch and heads
        mask = self.causal_mask[:T, :T].unsqueeze(0).unsqueeze(0)

        # Fill masked positions with the minimum representable value
        # (safe for fp16 / bf16; avoids NaNs)
        att = att.masked_fill(
            ~mask,
